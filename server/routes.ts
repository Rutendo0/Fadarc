...
// Right after your Cloudinary config in routes.ts
...
if (!req.file) {
      ...
      console.error('Cloudinary Config:', cloudinary.config());
      return res.status(500).json({ 
       ...
       const uploadPromise = uploadToCloudinary(req.file);
    ...
    const imageUrl = await Promise.race([uploadPromise, timeoutPromise]);
      ...
      fileName: `blog_${Date.now()}_${path.parse(req.file.originalname).name}`,
      fileSize: req.file.size,
      mimeType: req.file.mimetype,
      cloudUrl: typeof imageUrl === 'string' ? imageUrl : null
    });
